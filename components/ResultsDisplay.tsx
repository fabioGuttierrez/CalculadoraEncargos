...
export const ResultsDisplay: React.FC<ResultsDisplayProps> = ({ results }) => {
  const handleExportCSV = () => {
    ...
    link.setAttribute("download", "resumo_calculo_clt.csv");
  ...
  const directChargesAndBenefits = results.employer.fgts + results.employer.transportationVoucherCost + results.employer.mealVoucherCost + results.employer.employerInss + results.employer.thirdPartyContributions;
  const fgtsRateLabel = results.inputs.contractType === 'apprentice' ? '2%' : '8%';
    ...
    <div className="space-y-8">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <ResultCard title="Custo Total da Empresa" icon={<BuildingIcon className="h-6 w-6 text-cyan-400"/>}>
            <ResultRow label="Salário Bruto" value={formatCurrency(results.employer.grossSalary)} />
            <ResultRow label="Encargos e Benefícios" value={formatCurrency(directChargesAndBenefits)} />
            <ResultRow label="Provisões Mensais" value={formatCurrency(results.employer.totalProvisions)} />
            <ResultRow label="Custo Total Mensal" value={formatCurrency(results.employer.totalCost)} isTotal />
        </ResultCard>
      </div>
      ...
      <div className="bg-gray-800 p-6 rounded-lg border border-gray-700">
        <div className="flex justify-between items-center mb-4">
            ...
            className="flex items-center bg-gray-700 hover:bg-gray-600 text-cyan-400 font-bold py-2 px-4 rounded-lg transition-colors duration-300 text-sm"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4">
            ...
            <div>
                ...
                {results.employer.employerInss > 0 && <ResultRow label="INSS Patronal (20%)" value={formatCurrency(results.employer.employerInss)} tooltipText="Contribuição previdenciária paga pela empresa, correspondente a 20% sobre a folha de pagamento (para Lucro Real/Presumido)." />}
                {results.employer.thirdPartyContributions > 0 && <ResultRow label="Terceiros (RAT+S)" value={formatCurrency(results.employer.thirdPartyContributions)} tooltipText="Contribuições para Risco Ambiental de Trabalho (RAT) e outras entidades (Sistema S). Alíquota média de 5.8%." />}
                ...
                {results.employer.mealVoucherCost > 0 && <ResultRow label="Vale-Refeição/Alimentação" value={formatCurrency(results.employer.mealVoucherCost)} tooltipText="Benefício opcional, sem desconto em folha neste simulador. Custo = Valor Diário x Dias Trabalhados." />}
                ...
                {results.employer.totalProvisions > 0 && (
                  <div className="mt-4 pt-4 border-t border-gray-700">
                    ...
                    {results.employer.thirteenthProvisionTaxes > 0 && <ResultRow label="Encargos s/ 13º (Prov.)" value={formatCurrency(results.employer.thirteenthProvisionTaxes)} tooltipText="Provisionamento dos encargos patronais (INSS, RAT, Terceiros) que incidem sobre o 13º salário." />}
